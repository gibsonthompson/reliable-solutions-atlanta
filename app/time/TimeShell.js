...
export default function TimeShell({ children }) {
  ...
  // register the new one scoped to /time/ only so it never claims admin/marketing pages.
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        await Promise.all(
          regs
            .filter(r => !r.scope.endsWith('/time/'))
            .map(r => r.unregister())
        )
        if (!cancelled) {
          await navigator.serviceWorker.register('/time-sw.js', { scope: '/time/' })
        }
      } catch (e) {}
    })()
