...
const STATUS_COLORS = {
  ...
  job_booked: 'bg-emerald-100 text-emerald-700', in_progress: 'bg-orange-100 text-orange-700',
...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (result.data) {
        ...
        setFollowUps(all.filter(s => {
          ...
          if (s.next_follow_up) {
            const f = new Date(s.next_follow_up); f.setHours(0, 0, 0, 0)
  ...
  return (
    <div className="px-4 py-4 sm:py-8">
      {user && <p className="text-sm text-gray-500 mb-4">Welcome back, {user.name.split(' ')[0]}</p>}

      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm"><p className="text-gray-500 text-xs sm:text-sm">{user?.role === 'member' ? 'My Leads' : 'Total Leads'}</p><p className="text-2xl sm:text-3xl font-bold text-[#273373] mt-1">{stats.total}</p></div>
          <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm"><p className="text-gray-500 text-xs sm:text-sm">New Leads</p><p className="text-2xl sm:text-3xl font-bold text-blue-600 mt-1">{stats.new}</p></div>
          <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm"><p className="text-gray-500 text-xs sm:text-sm">This Week</p><p className="text-2xl sm:text-3xl font-bold text-[#115997] mt-1">{stats.thisWeek}</p></div>
          <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm"><p className="text-gray-500 text-xs sm:text-sm">Follow-ups Due</p><p className="text-2xl sm:text-3xl font-bold text-amber-600 mt-1">{followUps.length}</p></div>
        </div>
      )}
        ...
        return (
          <div className="bg-white rounded-xl shadow-sm mb-6 sm:mb-8 overflow-hidden">
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-2"><svg className="w-4 h-4 text-[#115997]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg><h2 className="text-base sm:text-lg font-semibold text-gray-800">Today&apos;s Schedule</h2><span className="text-gray-400 text-sm">({todayEvents.length})</span></div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-6 sm:mb-8">
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex items-center justify-between"><h2 className="text-base sm:text-lg font-semibold text-gray-800">Recent Submissions</h2><Link href="/admin/contacts" className="text-sm text-[#115997] font-medium hover:underline">View all</Link></div>
        {submissions.length === 0 ? <div className="p-8 text-center"><p className="text-gray-500">No submissions yet</p></div> : (
          <div className="divide-y divide-gray-100">
            {submissions.slice(0, 10).map((s) => (
              <Link key={s.id} href={'/admin/contacts/' + s.id} className="flex items-center justify-between p-4 sm:px-6 hover:bg-gray-50">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1"><p className="font-semibold text-gray-900 text-sm truncate">{s.name}</p><span className={'inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ' + (STATUS_COLORS[s.status] || 'bg-gray-100 text-gray-700')}>{STATUS_LABELS[s.status] || s.status}</span></div>
      ...
      {followUps.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm mb-6 sm:mb-8 overflow-hidden">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex items-center gap-2"><div className="w-2 h-2 bg-amber-500 rounded-full animate-pulse" /><h2 className="text-base sm:text-lg font-semibold text-gray-800">Follow-ups Due</h2><span className="text-gray-400 text-sm">({followUps.length})</span></div>
          <div className="divide-y divide-gray-100">
            {followUps.slice(0, 5).map((item) => (
              <Link key={item.id} href={'/admin/contacts/' + item.id} className="flex items-center justify-between p-4 sm:px-6 hover:bg-gray-50">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1"><p className="font-semibold text-gray-900 text-sm truncate">{item.name}</p><span className={'inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ' + (STATUS_COLORS[item.status] || 'bg-gray-100 text-gray-700')}>{STATUS_LABELS[item.status] || item.status}</span></div>
          ...
          <div className="space-y-3">
            {PIPELINE_BARS.map((stage) => (
              <div key={stage.key} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden"><div className={stage.color + ' h-full rounded-full transition-all duration-500'} style={{ width: stats.total > 0 ? Math.max(((stats[stage.key] || 0) / stats.total) * 100, stats[stage.key] > 0 ? 4 : 0) + '%' : '0%' }} /></div>
