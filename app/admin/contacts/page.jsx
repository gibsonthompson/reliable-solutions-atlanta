...
export default function ContactsPage() {
  ...
  const filtered = submissions
    .filter(s => {
      ...
      if (search) {
        ...
        return s.name?.toLowerCase().includes(q) || s.email?.toLowerCase().includes(q) || s.phone?.includes(q) || s.service_type?.toLowerCase().includes(q)
      }
      return true
    })
    .sort((a, b) => {
  ...
  const getUrgencyColor = (contact) => {
    ...
    const hours = (Date.now() - new Date(contact.created_at)) / 36e5
  ...
  return (
    <div className="px-4 py-4 sm:py-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-white rounded-xl p-3 shadow-sm">
          <p className="text-xs text-gray-500">Jobs Active</p>
        ...
        <div className="flex gap-2 min-w-max sm:min-w-0 sm:flex-wrap">
          {statuses.map((status) => (
            <button key={status.value} onClick={() => setFilter(status.value)}
              className={'flex-shrink-0 px-3 sm:px-4 py-2 rounded-full text-sm font-medium transition-all ' + (filter === status.value ? 'bg-[#115997] text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 active:scale-95')}>
              {status.label}
              <span className={'ml-1.5 ' + (filter === status.value ? 'text-white/70' : 'text-gray-400')}>{getStatusCount(status.value)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="text-sm text-gray-600 border-none bg-transparent focus:ring-0 cursor-pointer pr-6">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {filtered.length === 0 ? (
          <div className="p-8 sm:p-12 text-center">
            <p className="text-gray-500">{search ? 'No contacts match your search' : 'No contacts found'}</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Scheduled</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((contact) => (
                    <tr key={contact.id} className={'hover:bg-gray-50 transition-colors ' + getUrgencyColor(contact)}>
                      ...
                      <td className="px-6 py-4">
                        <span className={'inline-flex px-2.5 py-1 rounded-full text-xs font-medium ' + getStatusBadge(contact.status)}>{getStatusLabel(contact.status)}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500">
                        {contact.scheduled_date ? formatDate(contact.scheduled_date) : '—'}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500">{timeAgo(contact.created_at)}</td>
                      <td className="px-6 py-4">
                        <Link href={'/admin/contacts/' + contact.id} className="text-[#115997] hover:text-[#273373] font-medium text-sm">View →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <Link key={contact.id} href={'/admin/contacts/' + contact.id} className={'block p-4 active:bg-gray-50 transition-colors ' + getUrgencyColor(contact)}>
                  <div className="flex items-start justify-between mb-1.5">
                    ...
                    <div className="flex flex-col items-end ml-3">
                      <span className={'inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ' + getStatusBadge(contact.status)}>{getStatusLabel(contact.status)}</span>
