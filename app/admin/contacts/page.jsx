...
export default function ContactsPage() {
  ...
  return (
    <div className="px-4 py-5 sm:py-8">
      ...
      <div className="mb-4 sm:mb-6 -mx-4 px-4 overflow-x-auto scrollbar-hide animate-[fadeUp_0.45s_ease-out]"><div className="flex gap-2 min-w-max sm:min-w-0 sm:flex-wrap">{statuses.map((status) => <button key={status.value} onClick={() => setFilter(status.value)} className={'flex-shrink-0 px-3 sm:px-4 py-2 rounded-full text-sm font-medium transition-all duration-150 ' + (filter === status.value ? 'bg-[#115997] text-white shadow-sm shadow-[#115997]/20' : 'bg-white text-gray-500 border border-gray-200 hover:border-gray-300 active:scale-95')}>{status.label}<span className={'ml-1.5 ' + (filter === status.value ? 'text-white/60' : 'text-gray-400')}>{getStatusCount(status.value)}</span></button>)}</div></div>
