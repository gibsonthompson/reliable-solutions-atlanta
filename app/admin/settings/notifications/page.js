...
export default function NotificationsPage() {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', email: '', notification_types: ['new_lead', 'booked'] })
  ...
  const handleNew = () => {
    ...
    setFormData({ name: '', phone: '', email: '', notification_types: ['new_lead', 'booked'] })
  }

  const handleEdit = (r) => {
    setEditing(r.id); setError('')
    setFormData({
      name: r.name || '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSuccessMsg(isNew ? 'Recipient added' : 'Recipient updated')
      setEditing(null); fetchRecipients()
  ...
  const handleToggle = async (r) => {
    ...
    try {
      ...
      setSuccessMsg(next ? `${r.name} enabled` : `${r.name} disabled`)
  ...
  const handleDelete = async (r) => {
    ...
    try {
      ...
      setSuccessMsg('Recipient deleted')
      setTimeout(() => setSuccessMsg(''), 2000)
    } catch (e) {}
  }
    ...
    <div className="px-4 py-5 sm:py-8">
      ...
      {editing && (
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-5 border-2 border-[#115997]/20 animate-[fadeUp_0.2s_ease-out]">
          <h3 className="font-bold text-gray-900 mb-4">{editing === 'new' ? 'Add Recipient' : 'Edit Recipient'}</h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {NOTIFICATION_TYPES.map(t => (
                  <button key={t.key} onClick={() => toggleType(t.key)} className={'flex items-center justify-between px-3.5 py-3 rounded-xl border text-sm transition-all ' + (formData.notification_types.includes(t.key) ? 'border-[#115997]/20 bg-[#115997]/[0.04]' : 'border-gray-200 bg-white')}>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button onClick={handleSave} disabled={saving} className="flex-1 sm:flex-none px-4 py-2.5 text-sm font-semibold text-white bg-[#115997] rounded-xl hover:bg-[#0d4a7a] disabled:opacity-40 shadow-sm shadow-[#115997]/20 transition-all">{saving ? 'Saving...' : editing === 'new' ? 'Add Recipient' : 'Save Changes'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 animate-[fadeUp_0.35s_ease-out]">
        {recipients.length === 0 ? (
          <div className="p-8 sm:p-12 text-center">
            ...
            <button onClick={handleNew} className="mt-3 text-sm text-[#115997] font-semibold hover:underline">Add your first recipient</button>
          </div>
        ) : (
          <div className="divide-y divide-gray-50">
            {recipients.map(r => (
              <div key={r.id} className={'p-4 sm:px-6 transition-colors hover:bg-gray-50/50 ' + (r.enabled ? '' : 'bg-gray-50/30')}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5 tabular-nums">{formatPhoneDisplay(r.phone)}{r.email ? ' · ' + r.email : ''}</p>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => handleToggle(r)} className="p-1.5" title={r.enabled ? 'Disable' : 'Enable'}>
