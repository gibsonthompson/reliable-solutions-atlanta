...
const COLUMNS = [
  { key: 'new', label: 'New', color: 'border-blue-400', bg: 'bg-blue-50', badge: 'bg-blue-100 text-blue-700', help: 'New leads from the website. They got an auto-confirmation text. Call them ASAP.' },
  { key: 'estimate_sent', label: 'Estimate Sent', color: 'border-indigo-400', bg: 'bg-indigo-50', badge: 'bg-indigo-100 text-indigo-700', help: 'You visited and sent the estimate. Follow up if no response in 3 days.' },
  { key: 'in_progress', label: 'In Progress', color: 'border-emerald-400', bg: 'bg-emerald-50', badge: 'bg-emerald-100 text-emerald-700', help: 'They said yes. Job is scheduled or underway. Create a Job from the contact page.' },
  { key: 'done', label: 'Done', color: 'border-green-400', bg: 'bg-green-50', badge: 'bg-green-100 text-green-700', help: 'Job complete. Send the Review Request SMS template to ask for a Google review.' },
...
export default function PipelinePage() {
  ...
  const getUrgencyDot = (c) => { if (!['new'].includes(c.status)) return null; const h = (Date.now() - new Date(c.created_at)) / 36e5; if (h > 24) return 'bg-red-500'; if (h > 1) return 'bg-yellow-500'; return 'bg-green-500' }
  ...
  // Use display status for column placement
  ...
  return (
    <div className="px-4 py-4 sm:py-6">
      <div className="flex items-center justify-between mb-4">
        <div><h2 className="text-lg sm:text-2xl font-bold text-[#273373]">{user?.role === 'member' ? 'My Pipeline' : 'Pipeline'}</h2><p className="text-gray-500 text-xs sm:text-sm">{submissions.length} total</p></div>
