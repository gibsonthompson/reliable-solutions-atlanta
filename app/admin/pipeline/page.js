...
const COLUMNS = [
  ...
  { key: 'closed_lost', label: 'Lost', color: 'border-red-400', bg: 'bg-red-50', badge: 'bg-red-100 text-red-700', help: 'They decided not to move forward.' },
]
...
export default function PipelinePage() {
  ...
  const updateStatus = async (contactId, newStatus, oldStatus) => {
    if (newStatus === 'closed_lost') { setShowCloseModal({ id: contactId, oldStatus }); setCloseReason(''); return }
  ...
  const handleCloseLost = async () => {
    ...
    setSubmissions(prev => prev.map(s => s.id === id ? { ...s, status: 'closed_lost' } : s))
    setShowCloseModal(null)
    try {
      await fetch('/api/contact', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status: 'closed_lost', close_reason: closeReason }) })
      await fetch('/api/admin/activity', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contact_id: id, action: 'status_change', old_value: oldStatus, new_value: 'closed_lost', note: 'Reason: ' + closeReason, user_id: user?.id }) })
  ...
  return (
    <div className="px-4 py-4 sm:py-6">
      <div className="flex items-center justify-between mb-4">
        <div><h2 className="text-lg sm:text-2xl font-bold text-[#273373]">{user?.role === 'member' ? 'My Pipeline' : 'Pipeline Board'}</h2><p className="text-gray-500 text-xs sm:text-sm">{submissions.length} leads</p></div>
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.key} className="bg-gray-100/80 rounded-xl">
              <div className={'px-3 py-2 border-t-2 rounded-t-xl flex items-center justify-between ' + col.color}><div className="flex items-center gap-2"><h3 className="text-xs font-semibold text-gray-800">{col.label}</h3><span className={'text-[10px] font-bold px-1.5 py-0.5 rounded-full ' + col.badge}>{cards.length}</span></div>{cards.length === 0 && <span className="text-[10px] text-gray-400">No leads</span>}</div>
              {cards.length > 0 && (
      ...
      {showHelp && <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowHelp(false)}><div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}><div className="p-5 border-b border-gray-100"><div className="w-8 h-1 bg-gray-300 rounded-full mx-auto mb-3 sm:hidden" /><div className="flex items-center justify-between"><h3 className="text-lg font-bold text-[#273373]">How the Pipeline Works</h3><button onClick={() => setShowHelp(false)} className="text-gray-400 hover:text-gray-600"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div></div><div className="p-5 space-y-6"><div><h4 className="text-sm font-semibold text-gray-800 mb-2">The basics</h4><p className="text-sm text-gray-600 leading-relaxed">Every lead moves through stages from left to right. When someone fills out the free estimate form on the website, they show up in "New" automatically and get a confirmation text. Your job is to move them through each stage as things progress.</p></div><div><h4 className="text-sm font-semibold text-gray-800 mb-2">How to move a lead</h4><p className="text-sm text-gray-600 leading-relaxed">On a computer, drag the card and drop it into the column you want. On your phone, tap the card and pick the new stage from the list.</p></div><div><h4 className="text-sm font-semibold text-gray-800 mb-2">What each stage means</h4><div className="space-y-3">{COLUMNS.map((col) => <div key={col.key} className="flex gap-3"><div className={'w-1 flex-shrink-0 rounded-full ' + col.color.replace('border-', 'bg-')} /><div><p className="text-sm font-medium text-gray-800">{col.label}</p><p className="text-xs text-gray-500 leading-relaxed">{col.help}</p></div></div>)}</div></div><div><h4 className="text-sm font-semibold text-gray-800 mb-2">The colored dots</h4><p className="text-sm text-gray-600 leading-relaxed">Green means the lead came in less than 1 hour ago. Yellow means between 1 and 24 hours. Red means over 24 hours. These only show on New and Contacted leads to help you prioritize who to call first.</p></div><div><h4 className="text-sm font-semibold text-gray-800 mb-2">What happens automatically</h4><p className="text-sm text-gray-600 leading-relaxed mb-3">You do not need to do anything for these. They happen on their own.</p><div className="bg-gray-50 rounded-lg p-3 space-y-2.5"><div><p className="text-xs font-medium text-gray-800">New lead comes in</p><p className="text-xs text-gray-500">Customer gets a confirmation text. You get a notification text with their info.</p></div><div className="border-t border-gray-200 pt-2.5"><p className="text-xs font-medium text-gray-800">Lead moved to "Done"</p><p className="text-xs text-gray-500">3 days later, the customer gets a text asking them to leave a Google review.</p></div></div></div><div><h4 className="text-sm font-semibold text-gray-800 mb-2">Need more details on a lead?</h4><p className="text-sm text-gray-600 leading-relaxed">Tap or click any lead's name to open their full page. From there you can call them, text them, send an email, add notes, set a schedule date, and see everything that has happened with that lead.</p></div></div><div className="p-5 border-t border-gray-100"><button onClick={() => setShowHelp(false)} className="w-full py-3 bg-[#115997] text-white rounded-xl font-semibold hover:bg-[#273373] transition-colors">Got it</button></div></div></div>}
    </div>
  )
}