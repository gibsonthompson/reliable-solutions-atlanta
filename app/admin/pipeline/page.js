...
import { PIPELINE_STAGES, STAGE_BY_KEY, CLOSE_REASONS, REVENUE_PROJECTION_STAGES } from '../../../lib/pipeline'
...
export default function PipelinePage() {
  ...
  const updateStatus = async (contactId, newStatus, oldStatus) => {
    ...
    try {
      ...
      const colLabel = STAGE_BY_KEY[newStatus]?.label || newStatus
  ...
  const getCardsForColumn = (colKey) => submissions.filter(s => (s.status || 'new') === colKey)
  const columnRevenue = (cards) => cards.reduce((sum, c) => sum + (parseFloat(c.quoted_amount) || 0), 0)

  const totalRevenue = submissions
    .filter(s => REVENUE_PROJECTION_STAGES.includes(s.status))
    .reduce((sum, s) => sum + (parseFloat(s.quoted_amount) || 0), 0)
  ...
  const renderCard = (contact, draggable = true) => {
    const dot = getUrgencyDot(contact)
    const hasQuote = contact.quoted_amount && parseFloat(contact.quoted_amount) > 0
    return (
      <div key={contact.id}
        draggable={draggable}
        onDragStart={draggable ? (e) => handleDragStart(e, contact.id) : undefined}
        onDragEnd={draggable ? handleDragEnd : undefined}
        onClick={!draggable ? () => setMoveModal(contact) : undefined}
        className={'bg-white rounded-lg p-2.5 shadow-sm border border-gray-100 transition-all duration-150 hover:shadow-md hover:-translate-y-0.5 group ' + (draggable ? 'cursor-grab active:cursor-grabbing' : 'active:bg-gray-50 cursor-pointer') + ' ' + (draggingId === contact.id ? 'opacity-30 scale-95 rotate-1' : '')}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-1.5 min-w-0">
            {dot && <div className={'w-2 h-2 rounded-full flex-shrink-0 ' + dot + (dot === 'bg-red-500' ? ' animate-pulse' : '')} />}
            <Link href={'/admin/contacts/' + contact.id} onClick={(e) => e.stopPropagation()} className="font-semibold text-[11px] text-gray-900 truncate hover:text-[#115997] transition-colors">{contact.name}</Link>
          </div>
        ...
        {contact.address && <p className="text-[9px] text-gray-300 truncate mt-0.5">{contact.address}</p>}
        <div className="flex items-center justify-between mt-1.5">
          ...
          {hasQuote && <span className="text-[9px] font-semibold text-gray-600 tabular-nums">{fmtMoney(contact.quoted_amount)}</span>}
        </div>
      </div>
    )
  }
    ...
    <div className="px-4 py-5 sm:py-6">
      <div className="flex items-center justify-between mb-5 animate-[fadeUp_0.3s_ease-out]">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{submissions.length} total{totalRevenue > 0 ? ` - ${fmtMoney(totalRevenue)} projected` : ''}</p>
        </div>
      ...
      {/* Desktop: 9-column horizontal scroll */}
      <div className="hidden sm:block -mx-4 px-4 pb-2 overflow-x-auto scrollbar-hide animate-[fadeUp_0.35s_ease-out]">
        <div className="flex gap-2.5" style={{ minWidth: 'max-content' }}>
          {PIPELINE_STAGES.map((col) => {
            const cards = getCardsForColumn(col.key)
            const revenue = REVENUE_PROJECTION_STAGES.includes(col.key) ? columnRevenue(cards) : 0
            return (
              <div key={col.key}
                className={'flex flex-col rounded-xl transition-all duration-200 w-[240px] flex-shrink-0 ' + (dragOverCol === col.key ? 'ring-2 ring-[#115997]/30 bg-[#115997]/[0.03] scale-[1.01]' : 'bg-white/60')}
                ...
                <div className="bg-white rounded-t-xl border-b border-gray-100 overflow-hidden">
                  ...
                  <div className="px-3 py-2.5">
                    ...
                    {revenue > 0 && <p className="text-[10px] text-gray-400 tabular-nums mt-0.5">{fmtMoney(revenue)}</p>}
                  </div>
                </div>
                <div className="p-1.5 space-y-1.5 min-h-[80px] max-h-[calc(100vh-220px)] overflow-y-auto scrollbar-hide">
                  {cards.length === 0 && (
                    <div className="flex items-center justify-center h-20">
                      <p className="text-[10px] text-gray-300">No leads</p>
                    </div>
                  )}
                  {cards.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map((contact) => renderCard(contact, true))}
                </div>
              </div>
            )
          })}
            ...
            <div key={col.key} className="rounded-xl overflow-hidden bg-white/60">
              <div className="bg-white border-b border-gray-100">
                ...
                <div className="px-4 py-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {revenue > 0 && <span className="text-[10px] text-gray-400 tabular-nums">{fmtMoney(revenue)}</span>}
                      <span className="text-[10px] font-semibold text-gray-400 tabular-nums">{cards.length}</span>
                    </div>
                  </div>
                </div>
              ...
              {cards.length > 0 && (
                <div className="p-2 space-y-2">
                  {cards.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map((contact) => renderCard(contact, false))}
                </div>
              )}
            </div>
          )
      ...
      {/* Mobile Move Modal */}
      {moveModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center" onClick={() => setMoveModal(null)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm max-h-[80vh] overflow-y-auto animate-[slideUp_0.2s_ease-out]" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100">
              <div className="w-8 h-1 bg-gray-300 rounded-full mx-auto mb-3 sm:hidden" />
              <p className="font-bold text-gray-900">{moveModal.name}</p>
              <p className="text-xs text-gray-400">{moveModal.service_type}</p>
            </div>
            <div className="p-2">
              {PIPELINE_STAGES.map((col) => (
                <button key={col.key} onClick={() => handleMobileMove(col.key)} className={'w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ' + ((moveModal.status || 'new') === col.key ? col.bg + ' ring-1 ring-inset ring-gray-200' : 'hover:bg-gray-50 active:bg-gray-100')}>
                  <div className={'w-3 h-3 rounded-full flex-shrink-0 ' + col.dot} />
                  <span className={'text-sm font-semibold ' + ((moveModal.status || 'new') === col.key ? 'text-gray-900' : 'text-gray-500')}>{col.label}</span>
                  {(moveModal.status || 'new') === col.key && <svg className="w-4 h-4 text-[#115997] ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                </button>
              ))}
            </div>
            <div className="p-4 border-t border-gray-100">
              <Link href={'/admin/contacts/' + moveModal.id} className="block w-full text-center py-2.5 text-sm font-semibold text-[#115997] bg-[#115997]/[0.06] rounded-xl" onClick={() => setMoveModal(null)}>Open Details</Link>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Close/Lost Modal */}
      {showCloseModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowCloseModal(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full animate-[fadeUp_0.2s_ease-out]" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-900 mb-4">Why was this lead lost?</h3>
            <div className="space-y-2 mb-6">
              {CLOSE_REASONS.map((r) => (
                <button key={r} onClick={() => setCloseReason(r)} className={'w-full text-left px-4 py-3 rounded-xl border text-sm font-medium transition-all ' + (closeReason === r ? 'border-red-300 bg-red-50 text-red-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50')}>{r}</button>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowCloseModal(null)} className="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200">Cancel</button>
              <button onClick={handleCloseLost} disabled={!closeReason} className="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-red-500 rounded-xl disabled:opacity-40 hover:bg-red-600">Mark Lost</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showHelp && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowHelp(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto animate-[slideUp_0.2s_ease-out]" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-gray-100">
              <div className="w-8 h-1 bg-gray-300 rounded-full mx-auto mb-3 sm:hidden" />
              <div className="flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900">How the Pipeline Works</h3><button onClick={() => setShowHelp(false)} className="text-gray-400 hover:text-gray-600"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div>
            </div>
            <div className="p-5 space-y-5">
              <div><h4 className="text-sm font-bold text-gray-800 mb-1">The basics</h4><p className="text-sm text-gray-500 leading-relaxed">Every lead moves left to right through 9 stages. Each stage maps to one clear next action. New leads appear automatically when someone fills out the website form.</p></div>
              <div><h4 className="text-sm font-bold text-gray-800 mb-1">How to move a lead</h4><p className="text-sm text-gray-500 leading-relaxed">On desktop, drag and drop between columns. On mobile, tap a card and pick a new stage. Moving to Lost asks for a reason.</p></div>
              <div><h4 className="text-sm font-bold text-gray-800 mb-1">Auto behavior</h4><p className="text-sm text-gray-500 leading-relaxed">Moving to In Progress auto-creates a Job record if one does not already exist. Customer-booked estimates from the public site land in Estimate Scheduled automatically.</p></div>
              <div><h4 className="text-sm font-bold text-gray-800 mb-2">Stages</h4><div className="space-y-3">{PIPELINE_STAGES.map((col) => <div key={col.key} className="flex gap-3 items-start"><div className={'w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1 ' + col.dot} /><div><p className="text-sm font-semibold text-gray-800">{col.label}</p><p className="text-xs text-gray-400">{col.help}</p></div></div>)}</div></div>
            </div>
